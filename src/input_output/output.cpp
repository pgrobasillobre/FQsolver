...
void Output::print_matrix_rhs(const std::string &title, const Solvent &solv, const std::vector<double> &rhs) const
{
    ...
    log_stream << std::string(left_padding, ' ') << title << "\n\n";
    log_stream << " " << sticks << "\n\n";
    log_stream << "    Atom name      Mol. Index          Chi          Potential          RHS\n";
    log_stream << "    ---------      ----------       ---------       ---------       ---------\n";
...
//----------------------------------------------------------------------
// Prints FQ charges by atom.
void Output::print_results(const std::string &title, const Solvent &solv, const std::vector<double> &results) const
{
    if (static_cast<int>(results.size()) < solv.natoms)
    {
        throw std::runtime_error("Cannot print FQ charges: result vector is smaller than the number of solvent atoms.");
    }
    ...
    log_stream << std::string(left_padding, ' ') << title << "\n\n";
    log_stream << " " << sticks << "\n\n";
    log_stream << "    Atom name      Mol. Index        Charges\n";
    log_stream << "    ---------      ----------       ---------\n";
    ...
    for (int i = 0; i < solv.natoms; ++i)
    {
        ...
        char line[120];
        std::snprintf(line, sizeof(line), "       %-5s        %5d        %12.6f\n",
                      solv.atomic_label[i].c_str(),
                      mol_index,
                      results[i]);
        log_stream << line;
...
//----------------------------------------------------------------------
// Writes FQ charges to the FQSolver_results directory.
void Output::print_results(const Target &target, const Solvent &solv, const std::vector<double> &results) const
{
    if (static_cast<int>(results.size()) < solv.natoms)
    {
        throw std::runtime_error("Cannot write FQ charges: result vector is smaller than the number of solvent atoms.");
    }

    if (static_cast<int>(solv.solv_pot.size()) != solv.natoms)
    {
        throw std::runtime_error("Cannot write FQ charges: solvent potential data are not available.");
    }
    ...
    const std::string base_filename = std::filesystem::path(target.input_filename).stem().string();
    const std::string output_filename = results_dir / (base_filename + "_charges.txt");
    ...
    const int mol_width = 10;
    const int value_width = 25;
    const std::string gap = " ";
    const int table_width = atom_width + mol_width + (5 * value_width) + (6 * static_cast<int>(gap.size())) + 3;
    ...
    outfile << "# ======================\n";
    outfile << "#\n";
    outfile << "# Results of computed FQ charges at solvent coordinates\n";
    outfile << "#\n";
    ...
    outfile << "#\n";
    outfile << "# parametrization: " << target.fq_parametrization << "\n";
    outfile << "# kernel: " << target.fq_kernel << "\n";
    outfile << "#\n";
    outfile << "# " << std::string(table_width, '-') << "\n";
    outfile << "# " << center("Atom", atom_width) << gap + "       "
            << center("Mol. Index", mol_width) << gap + " "
            << center("X (Å)", value_width) << gap + " "
            << center("Y (Å)", value_width) << gap
            << center("Z (Å)", value_width) << gap + " "
            << center("Potential (a.u.)", value_width)
            << center("Charge (a.u.)", value_width) << "\n";
    outfile << "# " << std::string(table_width, '-') << "\n";
    ...
    for (int i = 0; i < solv.natoms; ++i)
    {
        ...
        outfile << std::left << "     " << std::setw(atom_width) << solv.atomic_label[i] << gap
                << std::right << std::setw(mol_width) << mol_index << gap
                ...
                << std::setw(value_width) << results[i] << "\n";
    }
    ...
    log_stream << " " << sticks << "\n\n";
    log_stream << std::string(5, ' ') << "Computed FQ charges at solvent coordinates have been saved to: \n\n";
    log_stream << std::string(5, ' ') << std::filesystem::absolute(output_filename).string() << "\n\n";
    log_stream << " " << sticks << "\n\n";
    log_stream.flush();
}
//----------------------------------------------------------------------
